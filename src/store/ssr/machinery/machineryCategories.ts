...
export async function getMachineryCategoies() {
	...
	const api = `${BASE_URL}/machinery-categories?${params.toString()}`;
	...
	if (!res.ok) {
		console.error(`Failed to fetch machineries, Status: ${res.status}`);
